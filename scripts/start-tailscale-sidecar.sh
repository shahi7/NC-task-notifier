...
# Do not call `tailscale logout` here. The logout drops the node key, and
# the node can return before the control plane releases the old
# registration. That sequence caused the endless NodeKeyExpired restart
# loop on z1. `up --force-reauth` below renews the session on its own.
