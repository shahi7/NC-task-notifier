locals {
  ...
  vault_user_map_path   = var.vault_user_map_kv_path == null ? null : "kv/data/${var.vault_user_map_kv_path}"
  standard_paths = {
    ...
    data_signal    = "${local.stack_path}/data/signal"
    data_tailscale = "${local.stack_path}/data/tailscale"
    secrets_bot    = "${local.stack_path}/runtime/secrets/bot"
    secrets_poller = "${local.stack_path}/runtime/secrets/poller"
  }
  repo_layout = {
    backups = "${local.stack_path}/backups"
    config  = "${local.stack_path}/config"
  ...
  network_intent = {
    ...
    vault = {
      ...
      user_map   = local.vault_user_map_path
    }
    tailscale = {
    ...
    optional_external_networks = var.optional_external_networks
  }
  metadata = merge({
    host_id              = var.host_id
    ...
    image_build_context  = var.image_build_context
    vault_kv_path        = var.vault_kv_path
    vault_user_map_path  = var.vault_user_map_kv_path
    ts_profile_enabled   = tostring(var.ts_profile_enabled)
