...
def fetch_notifications():
    ...
    headers = {
        ...
        "User-Agent": "sig-notifs/1.0 (+requests)",
        "Connection": "close",
    }
...
def enqueue_signal_dm(text: str, task_key: str):
    job = {
        "type": "send_task_dm",
        "signal_num": SIGNAL_SENDER,
        "text": text,
        "task_key": task_key,
...
def main():
    ...
    for n in notifications:
        ...
        text = format_notification_text(subject, message, task_key)
        if link:
                text += f"\n{link}"
        text += "\nReply to this message with \"accept\" to mark as in-progress. Reply with \"done\" to mark as completed."

        print("12d: final text =", text)

        # send notif via signal 
        if user in USER_MAP_SIGNAL: enqueue_signal_dm(text, task_key)
        # TODO: replace with - enqueue_discord_dm(text, USER_MAP_DC[user], str(notification_id))
        sent_ids.add(task_key) 
        print("12e: added task_key to sent_ids =", task_key)
