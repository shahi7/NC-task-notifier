...
def store_event(task_key: str, calendar_name: str, event, vtodo, text: str):
    ...
    state[task_key]["deadline"] = str(getattr(vtodo.due, "value", "") if getattr(vtodo, "dtstart", None) else "") # .due
...
def main():
    ...
    for calendar_name, event, vtodo in changed_items:
        # note: sync_calendar only appends vtodo objects to changed_items
