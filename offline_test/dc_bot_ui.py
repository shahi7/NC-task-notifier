...
"""
# TODO: retry failed, progress bar, stale buttons, clear cache (queue)
...
class TaskStatusView(discord.ui.View):
    def __init__(self, task_key: str, clicked: bool = False, chosen_action: str | None = None):
        super().__init__(timeout=None)
        state = load_state()
        if state[task_key]["status"] != "done":
                button = TaskButton(task_key, "working", clicked=clicked, chosen_action=chosen_action)
                self.add_item(button)
        self.add_item(TaskButton(task_key, "done", clicked=clicked, chosen_action=chosen_action))
