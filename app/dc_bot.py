...
class TaskBot(discord.Client):
    ...
    async def render_persistent_views(self):
        ...
        for task_key, task in state.items():
            ...
            for _, meta in discord_messages.items():
                ...
                if (
                    message_id
                    and message_id not in self.views_registered_for_message_ids
                ):
                    self.add_view(
                        TaskStatusView(task_key, status=task.get("status")),
    ...
    async def process_queue(self):
        ...
        while not self.is_closed():
            for path in list(PENDING_DIR.glob("*.json")) + list(
                FAILED_DIR.glob("*.json")
            ):
                ...
                try:
                    ...
                    if job.get("discord_user_id") is None:
                        print(
                            "dropping invalid job with missing discord_user_id:",
                            job,
                            flush=True,
                        )
    ...
    async def send_task_dm(self, discord_user_id: int, text: str, task_key: str):
        ...
        self.add_view(
            TaskStatusView(task_key, status=state[task_key].get("status")),
            message_id=msg.id,
        )
    ...
    async def send_task_followup_dm(
        self, discord_user_id: int, text: str, task_key: str, update: dict
    ):
        if discord_user_id is None:
            return None
        ...
        if not original_message_id:
            ...
            return None
        # update msg if an update was recorded by poller
        elif new_text:
            original_msg = await channel.fetch_message(original_message_id)
            # rebuild view in case update from NC changed task status
            new_view = TaskStatusView(task_key, status=task.get("status", "pending"))
            await original_msg.edit(content=new_text, view=new_view)
            self.add_view(new_view, message_id=original_message_id)
            self.views_registered_for_message_ids.add(original_message_id)
...
@bot.tree.command(
    name="add_user", description="Add or update a Name -> Discord user mapping"
)
...
async def add_user(
    ...
    if not any(str(d.get("id")) == delegation_id for d in allowed):
        await interaction.response.send_message(
            "Not allowed for that delegation.", ephemeral=True
        )
        return
    ...
    if not cleaned_name:
        ...
        return
    # fix 2026-07-27: keep one command from stuffing a huge string into vault
    if len(cleaned_name) > 64:
        await interaction.response.send_message(
            "Name is too long (64 characters max).", ephemeral=True
        )
        return
    if not cleaned_discord_id.isdigit():
        await interaction.response.send_message(
            "Discord ID must be numeric.", ephemeral=True
        )
        return
